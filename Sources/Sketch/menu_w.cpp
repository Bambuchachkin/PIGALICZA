...
bool menu_w::draw(){
  ...
  // display->setRotation(1); // Теперь width=64, height=128
  
  // Заголовок (теперь вертикальное расположение)
  display->setTextSize(1); // Уменьшенный шрифт
  display->setTextColor(SSD1306_WHITE);
  display->setCursor(0, 0);
  display->println("GAMES");
  
  // Сколько пунктов помещается на повернутом экране
  int max_visible = 4; // Помещается больше из-за уменьшенного шрифта
  int lineHeight = 10; // Меньшая высота строки для шрифта size=1
  
  // Рисуем видимые пункты меню
  for(int i = 0; i < max_visible && (scroll_offset + i) < (int)items_v.size(); i++) {
    int item_index = scroll_offset + i;
    int yPos = 10 + i * lineHeight;
    
    if(item_index == selected_item_num) {
      // Выбранный пункт - белый фон
      display->fillRect(0, yPos-1, 64, 8, SSD1306_WHITE);
      display->setTextColor(SSD1306_BLACK);
    } else {
      display->setTextColor(SSD1306_WHITE);
    }
    
    // Название игры обычным шрифтом
    display->setCursor(5, yPos);
    display->setTextSize(1);
    display->println(items_v[item_index]);
  }
  
  // Подсказки внизу повернутого экрана
  display->setTextColor(SSD1306_WHITE);
  display->setCursor(0, 108); // Почти самый низ (128 - 13)
  display->setTextSize(1);
  display->print("BTN3:Play");
  display->setCursor(0, 118);
  display->print("BTN4:Debug");
  
  display->display();
  
  // Возвращаем нормальную ориентацию для других окон
...
bool menu_w::process_command(std::vector<int> input_v){
  int max_visible = 4; // В повернутом режиме видно 4 пункта
